    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                {{ $subscribeTransactions->links() }}
                @forelse ($subscribeTransactions as $subscribeTransaction)
                    <div class="item-card flex flex-row justify-between items-center">
                        <svg class="hidden md:inline" width="46" height="46" viewBox="0 0 24 24" fill="none"
                        ...
                        <div>
                            ...
                            @if ($subscribeTransaction->is_paid)
                                <span class="px-4 py-1 bg-green-500 text-white font-bold rounded-full text-sm">
                                    ACTIVE
                                </span>
                            @else
                                <span class="px-4 py-1 bg-orange-500 text-white font-bold rounded-full text-sm">PENDING
                                </span>
                            @endif
                        </div>
                        <div class="hidden md:flex flex-col">
                        ...
                        <div class="flex flex-row items-center gap-x-3">
                            <a href="{{ route('admin.subscribe_transactions.show', $subscribeTransaction) }}"
                ...
                @empty
                    <p class="text-xl font-bold">Belum ada data transaksi</p>
                @endforelse
            </div>
        </div>
    </div>
