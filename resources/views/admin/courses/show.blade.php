    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                <div class="item-card flex flex-row gap-y-10 gap-x-3 justify-between items-center">
                    <div class="flex flex-row items-center gap-x-3">
                        <img src="{{ Storage::url($course->thumbnail) }}" alt="{{ $course->name }}"
                            class="rounded-2xl object-cover w-[200px] h-[150px] hidden md:inline">
                ...
                <div class="flex flex-row justify-between items-center">
                    <div class="flex flex-col">
                        ...
                        <p class="text-slate-500 text-sm">{{ $course->courseVideos->count() }}</p>
                    </div>
                ...
                @forelse($courseVideos as $courseVideo)
                    <div class="item-card flex flex-col md:flex-row gap-y-2 md:gap-y-10 justify-between items-center">
                        <div class="flex flex-row items-center gap-x-3">
                            <iframe width="560" class="rounded-2xl object-cover w-[160px] h-[90px]" height="315"
                                src="https://www.youtube.com/embed/{{ $courseVideo->path_video }}"
                        ...
                        <div class="flex flex-row items-center w-full md:w-auto gap-x-3">
                            <a href="{{ route('admin.course_videos.edit', $courseVideo) }}"
                                class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
                                Edit Video
                            </a>
                            <form action="{{ route('admin.course_videos.destroy', $courseVideo) }}" method="POST">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <p class="text-xl font-bold">Belum ada video course</p>
                @endforelse
